...
const AllUsersUniqueXylo = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const allUsersResponse = await axios.get('http://localhost:8080/rooms/alluser');
        const usersData = allUsersResponse.data;

        const detailedUsers = await Promise.all(
          usersData.map(async (user) => {
            try {
              const userDetailsResponse = await axios.get(`http://localhost:8080/usercontrol/${user.username}`);
